...
const TypeGroup = ['Rename', 'Furniture', 'Model', 'Select', 'Panel'];
...
// Параметризация окна
const WIDTH_BUTTON = 260;
const HEIGHT_BUTTON = 30;
const WIDTH_ALIGN = 10;
const HEIGHT_ALIGN = 10;
const WIDTH_FORM =
  TypeGroup.length * WIDTH_BUTTON + (TypeGroup.length + 1) * WIDTH_ALIGN;
// Определение списка параметров окна
...
for (let i = 0; i < TypeGroup.length; i++) {
  ...
  for (let j = 0; j < InitButton.length; j++) {
    if (InitButton[j].GroupType == TypeGroup[i]) {
      let CurrentName = InitButton[j].Name;
      let CurrentDescription = InitButton[j].Description;
...
ScriptForm.SearchOboz.OnClick = () => {
  ...
  if (Elements.length > 0) {
    Model.forEach((objects) => {
      if (objects instanceof TExtrusionBody || objects instanceof TFurnPanel || objects instanceof TFurnBlock) {
